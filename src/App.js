...
const SERVER_URL = "https://quiet-falls-42399.herokuapp.com";
...
const
    ...
    CaseDialog = props => {
        ...
        return (
            ...
            >
                <WaitingDialog users={props.users} open={Boolean(challengeID !== "" && (timer === null || (timer && timer < 0)))} onClose={() => {
                    if (ownerID === props.socketID) { cancelChallenge(); setChallengeID(""); setTimer(null) } else { leaveChallenge(); }
                }} members={members} setTimer={setTimer} timer={timer} challengeID={challengeID} startChallenge={startChallenge} owner={ownerID === props.socketID} ownerID={ownerID} />
                <ResultsDialog winners={winners} open={challengeID !== "" && winners.length > 0} onClose={() => { setChallengeID(""); setTimer(null) }} socketID={props.socketID} users={props.users} results={results} />
                <DialogContent>
    ...
    VerbDialog = props => {
        ...
        useEffect(() => {
            ...
            socket.on("challenge started", _id => {
                //setChallengeID(id);
                setTimer(-3000);
            });
            socket.on("challenge finished", (id, time) => {
...
export default () => {
    ...
    const tabs = ["Nouns", "Adjectives", "Pronouns", "Numbers", "Participles", "Actives", "Passives", "verbs other forms", "Live", "Settings"/*, "Memes"*/];
    return (
        <ThemeProvider theme={theme}>
            {localStorage.getItem("used") !== "yes" && <FirstTimeDialog set={(us, s) => { setName(us); setSchool(s); socketJoin(us); }} />}
            <Dialog
                open={resetOpen}
                maxWidth="xl"
                fullWidth
            ...
            <Snackbar
                ...
                action={
                    <React.Fragment>
                        <Button style={{ color: "black" }} size="small" onClick={() => { setValue(tabs.indexOf("Live")); setNounOpen(false); setOpen(null) }}>
                            View
                        </Button>
